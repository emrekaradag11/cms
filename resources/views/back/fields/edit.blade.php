<form action="{{route('admin.updateFieldData')}}" class="card-body pb-3" method="post">
    @csrf
    ...
    <div class="tab-content col-12 m-0">
                    ...
                    @foreach($fields as $f)
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="{{$f->id}}[]" class="form-control">
                                        @foreach(explode(',', $f->fieldDetail[$l]->properties) as $pr)
                                            <option {{(!empty($fieldData[$f->id][$l]["text"]) && ($fieldData[$f->id][$l]["text"] == $pr))?"selected":null}} value="{{@trim($pr)}}">{{@trim($pr)}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                    ...
                                    <div class="d-block">
                                        @foreach(explode(',', $f->fieldDetail[$l]->properties) as $ks => $pr)
                                            <div class="custom-control pr-2 mb-2 d-inline-block custom-radio">
                                                <input type="radio" {{(!empty($fieldData[$f->id][$l]["text"]) && ($fieldData[$f->id][$l]["text"] == $pr))?"checked":null}} value="{{$pr}}" id="custom{{$ks . $k->lang_short}}" name="{{$f->id}}[]" class="custom-control-input">
                        ...
                        @else
                            <div class="col-12">
                                <div class="form-group">
                                    <label>{{$f->fieldDetail[$l]->name}}</label>
                                    <input value="{{!empty($fieldData[$f->id][$l]["text"])?$fieldData[$f->id][$l]["text"]:null}}" type="{{$f->type}}" name="{{$f->id}}[]" class="form-control">
                                </div>
                            </div>
                        @endif
                    @endforeach
        ...
        <div class="col-12">
            <div class="form-group text-right">
                ...
                <input type="hidden" name="parent_id" value="{{$field_parent}}">
